...
###############################################################
# 게임 성공 창 함수 
def YouWin(game_result,Level):              # 위에서 받은 game_result 값을 받아 실행됨
    while True:
        game_font = pygame.font.Font(None, 40)
        ...
        for event in pygame.event.get():    # 이벤트가 발생하였는가?
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if Button_Next_Level.checkForInput(Gameover_MosPos):                # Continue버튼 눌리면  함수 실행
                        pygame.time.delay(1000)
                        if Level == 1:                                      #레벨 1이면 컨티뉴 버튼 누를 시 레벨 2 게임 실행
                            BallGame_Lv_2()
                            
                        elif Level == 2:                                    #레벨 2이면 컨티뉴 버튼 누를 시 레벨 3 게임 실행
                            BallGame_Lv_3()
...
def BallGame_LV_1():
    ...
    if running != True and Gameover :
        ...
        pygame.time.delay(2000)     # 2초 대기
        YouWin(game_result,Level)         # 게임 성공 함수 실행
    pygame.display.update()
