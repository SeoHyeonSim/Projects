...
# 게임오버 메시지 표시       
def Gameover_shooting_screen():
    while True:
        shooting_screen.blit(Gameover_text,Gamover_rect)
...
#게임 클리어 함수
def Gameclear_shooting_screen():
    while True:
        shooting_screen.blit(Gameclear_text,Gameclear_rect)
...
# 운석 생성 부분    
        ...
        if random.randint(1,pod_prob) == 1:
            for i in range(occur_of_rock):
                speed = 4
                pod = EscapePod(random.randint(0,shooting_screen_width - 30),0,speed)
                pods.add(pod)
...
# 게임 종료 조건 설정
    ...
    if Gameover:
        ...
        Gameover_shooting_screen()
        
        
    # 게임 클리어 메시지
    if Gameclear:
        Gameclear_shooting_screen()
