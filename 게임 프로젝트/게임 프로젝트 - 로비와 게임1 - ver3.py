...
def startScreen():
        ...
        while True:
            ...
            Button_Back = Button(button_surface, 400, 550, "Quit","white","yellow")
...
# 게임오버 함수
def GAMEOVER(game_result):              # 위에서 받은 game_result 값을 받아 실행됨
    while True:
        ...
        for button in [Button_Continue,Button_Back]:
        ...
        for event in pygame.event.get():    # 이벤트가 발생하였는가?
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if Button_Continue.checkForInput(Gameover_MosPos):                # Continue버튼 눌리면  함수 실행
                        ...
                        BallGame_LV_1()
                    elif Button_Back.checkForInput(Gameover_MosPos):                 # quit버튼 눌리면 게임 종료
...
###############################################################
# 게임 성공 창 함수 
def YouWin(game_result):              # 위에서 받은 game_result 값을 받아 실행됨
    while True:
        game_font = pygame.font.Font(None, 40)
        ...
        for button in [Button_Next_Level,Button_Back]:
            button.changeColor(Gameover_MosPos)
            button.update()
...
def BallGame_LV_1():
    ...
    if running != True and Gameover :
        ...
        pygame.time.delay(2000)     # 2초 대기
        YouWin(game_result)         # 게임 성공 함수 실행
    pygame.display.update()
...
# 게임 종료 출력
    if running != True:
        pygame.time.delay(2000)     # 2초 대기
        GAMEOVER(game_result)       # 게임오버 함수 실행 - 마지막 레벨이니 뭐.... 
    pygame.display.update()
